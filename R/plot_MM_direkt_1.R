...
plot_MM_direct <- function(sub, velo, title = "title", xlab = "abscisaa", ylab = "ordinate"){
    ...
    # plotten des Fits
       ...
       ggplot2::geom_text(ggplot2::aes(0,stats::coef(fit)[[1]],label = round(coef(fit)[[1]], digits = 3), vjust = 1.4), hjust = .3 )+ # Vmax aus coefficents eintragen
       ...
       ggplot2::geom_text(ggplot2::aes(0,coef(fit)[[2]],label = round(stats::coef(fit)[[2]], digits = 3), vjust = 1.41), hjust = -2.3)+ # Km aus coefficents eintragen
       ggplot2::xlab(xlab)+
       ggplot2::ylab(ylab)+
